...
// The script now dynamically gets the GitHub username and the start year from environment variables.
const USERNAME = process.env.USERNAME
const SINCE_YEAR = process.env.SINCE_YEAR
	? parseInt(process.env.SINCE_YEAR)
	: null
...
async function fetchContributions(startYear, prCache) {
	// Enhanced input validation to provide clear error messages.
	...
	if (!USERNAME) {
		throw new Error(
			"USERNAME is not set. Please add USERNAME as a repository secret."
		)
	}
	if (!SINCE_YEAR) {
		throw new Error(
			"SINCE_YEAR is not set. Please add SINCE_YEAR as a repository secret."
		)
	...
	const currentYear = new Date().getFullYear() // Loop through each year from the start year to the current year.

	for (let year = startYear; year <= currentYear; year++) {
		console.log(`Fetching contributions for year: ${year}...`) // Define the start and end dates for the year to use in API queries.
		...
		async function getAllPages(query) {
			...
			while (true) {
				try {
					...
					results.push(...response.data.items) // Check for a 'next' page link in the headers.
		...
		const prs = await getAllPages(
			`is:pr author:${USERNAME} is:merged merged:>=${yearStart} merged:<${yearEnd}`
		)
		...
		for (const pr of prs) {
			...
			const repoName = repoParts[repoParts.length - 1] // 2. Check if the PR is from your own repo.
			...
			prCache.add(pr.html_url) // 4. Check the temporary, in-run cache to avoid processing the same PR twice within this run.
		...
		const issues = await getAllPages(
			`is:issue author:${USERNAME} -user:${USERNAME} created:>=${yearStart} created:<${yearEnd}`
		)
		...
		} // --- Fetch Reviewed PRs (PRs reviewed, merged, or closed by the user) --- // Note: The GitHub search API doesn't support multiple 'closed-by' or 'merged-by' filters, // so we combine multiple queries and deduplicate the results.

		const reviewedByPrs = await getAllPages(
			`is:pr reviewed-by:${USERNAME} -author:${USERNAME} updated:>=${yearStart} updated:<${yearEnd}`
		)
		const mergedByPrs = await getAllPages(
			`is:pr merged-by:${USERNAME} -author:${USERNAME} updated:>=${yearStart} updated:<${yearEnd}`
		)
		const closedByPrs = await getAllPages(
			`is:pr is:closed -author:${USERNAME} closed-by:${USERNAME} commenter:${USERNAME} closed:>=${yearStart} closed:<${yearEnd}`
		)
		...
		for (const pr of combinedResults) {
			// New check to skip any PRs created by a bot.
		...
		const collaborationsPrs = await getAllPages(
			`is:pr is:open commenter:${USERNAME} -author:${USERNAME} -reviewed-by:${USERNAME} updated:>=${yearStart} updated:<${yearEnd}`
		)
		const collaborationsIssues = await getAllPages(
			`is:issue commenter:${USERNAME} -author:${USERNAME} updated:>=${yearStart} updated:<${yearEnd}`
		)
...
function groupContributionsByQuarter(contributions) {
	const grouped = {} // Iterate over each contribution type (pullRequests, issues, etc.).
	for (const [type, items] of Object.entries(contributions)) {
		...
		for (const item of items) {
			...
			const month = dateObj.getMonth() + 1 // Calculate the quarter (1-4).
			const quarter = `Q${Math.floor((month - 1) / 3) + 1}`
			const key = `${year}-${quarter}` // Initialize the quarter group if it doesn't exist.
...
async function writeMarkdownFiles(groupedContributions) {
	const baseDir = "test-contributions" // Create the base contributions directory if it's doesn't exist.
	await fs.mkdir(baseDir, { recursive: true }) // Iterate over each quarter's worth of data.
	...
	for (const [key, data] of Object.entries(groupedContributions)) {
		...
		const filePath = path.join(yearDir, `${quarter}-${year}.md`) // Calculate the total number of contributions for the quarter.
		...
		const top3Repos = sortedRepos.slice(0, 3) // --- Start building the Markdown content with a main header ---
...
async function main() {
	try {
		...
		const dataDir = "test-data" // Ensure the data directory exists before trying to read from or write to it.
		await fs.mkdir(dataDir, { recursive: true }) // Use the path module to correctly build the file paths.

		const cacheFile = path.join(dataDir, "pr-cache.json")
		const dataFile = path.join(dataDir, "all-contributions.json")

		let prCache = new Set() // Try to load the cache from a JSON file.
		...
		console.log(`Fetching new contributions since: ${mostRecentDate}`) // Fetch new contributions and update the cache.

		const { contributions: newContributions, prCache: updatedPrCache } =
			await fetchContributions(new Date(mostRecentDate).getFullYear(), prCache) // Merge the new contributions with the existing ones.
		...
		console.log("Merged new contributions with existing data.") // Save the updated, full contributions data to a new JSON file
		...
		await writeMarkdownFiles(grouped) // Save the updated cache to a file for future runs.
